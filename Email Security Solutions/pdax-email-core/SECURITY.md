...
- The SEGS dashboard application (`server/`)
- The Gmail receiver service (`gateway/`)
- The email analysis pipeline (`app/pipeline/`)
- Deployment scripts and container configuration (`deploy/`, `Dockerfile*`, `ecs/`)
- Authentication and session management (`server/auth_store.py`, `server/security.py`)
