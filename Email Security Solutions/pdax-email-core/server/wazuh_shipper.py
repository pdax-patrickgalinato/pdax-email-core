...
_SOURCES = {
    "activity_audit": _ROOT / "data" / "activity_audit.jsonl",
    "shadow_enforcement": (
        Path(os.environ.get("SEG_QUARANTINE_ROOT", str(_ROOT / "gateway" / "spool")))
        / "shadow_logs" / "shadow_enforcement.jsonl"
    ),
}

_CHECKPOINT_PATH = _ROOT / "data" / "wazuh_shipper_offsets.json"
...
def start_shipper() -> None:
    ...
    bucket = os.environ.get("SEG_S3_BUCKET", "").strip()
    if not bucket:
        _log.info("wazuh_shipper: disabled (SEG_S3_BUCKET not set)")
        return
    prefix = os.environ.get("SEG_S3_PREFIX", "segs/logs").strip()
    region = (
        os.environ.get("SEG_S3_REGION", "").strip()
        or os.environ.get("AWS_REGION", "").strip()
        or "ap-southeast-1"
    )
    interval = max(10, int(os.environ.get("SEG_S3_SHIP_INTERVAL", "60")))
    t = threading.Thread(
