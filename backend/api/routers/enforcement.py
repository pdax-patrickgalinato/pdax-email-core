...
Reads and writes backend/policy/runtime/enforcement_mode.yaml so the gateway's disposition
...
blocking (quarantine / reject) is disabled on this Gmail read-only
deployment — PUT only accepts ``shadow``.
  ...
  reject     — hard-reject MALICIOUS (requires allow_reject_on_malicious in
               backend/policy/detection/disposition.yaml; downgraded to quarantine otherwise)
"""
...
_ENFORCE_FILE = RULES_RUNTIME / "enforcement_mode.yaml"
...
def set_enforcement(body: EnforcementUpdate, user: User = Depends(require_role("admin"))):
    ...
    if mode != "shadow":
        raise HTTPException(
            status_code=409,
            detail="This deployment is monitor-only (Gmail read-only). Mail is never quarantined or rejected.",
        )
