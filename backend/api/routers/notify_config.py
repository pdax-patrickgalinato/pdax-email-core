...
Reads and writes backend/policy/runtime/notify_config.yaml. The SMTP password is write-only:
...
_CONFIG_FILE = RULES_RUNTIME / "notify_config.yaml"
...
def get_notify_config(user: User = Depends(require_role("admin"))):
    ...
    cfg["smtp_pass_set"] = bool(get_settings().notify_smtp_pass.strip())
    return cfg
