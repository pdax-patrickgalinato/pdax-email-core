...
GET is a plain read of backend/policy/detection/policy.yaml, re-shaped to match what
...
workers/pipeline/policy.py itself stays read-only (matches its own docstring
framing) — this router is the only place in the codebase with a policy
...
from backend.paths import RULES_DETECTION

_POLICY_PATH = RULES_DETECTION / "policy.yaml"
...
def put_policy(body: PolicyUpdate, admin: User = Depends(require_role("admin"))):
    ...
    if not _POLICY_PATH.is_file():
        raise HTTPException(status_code=500, detail="backend/policy/detection/policy.yaml is missing")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Policy update failed — category entry not found in backend/policy/detection/policy.yaml")
