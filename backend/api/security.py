...
retry_ai_limiter = RateLimiter(max_attempts=20, window_seconds=60)
search_limiter = RateLimiter(max_attempts=30, window_seconds=60)  # spotlight NL → SQL
...
_BODY_SIZE_LIMIT = int(get_settings().max_body_bytes)
...
# Scripts are bundled by Vite (no inline <script>). Inline style="" attributes
# in the console markup still need 'unsafe-inline' on style-src.
_CSP = (
    "default-src 'self'; "
    "script-src 'self'; "
...
_SSO_PROVIDER = get_settings().sso_provider.strip().lower()
...
#   /scim/v2/*        — IdP provisioning (RFC 6750 bearer, not OIDC)
_SSO_BYPASS_PREFIXES = ("/api/health", "/api/auth/", "/api/setup", "/scim/")
...
class SSOMiddleware(BaseHTTPMiddleware):
    ...
    and configure the ALB OIDC listener rule as described in docs/jumpcloud-sso.md.
    """
