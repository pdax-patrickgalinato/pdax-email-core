...
Architecture (claude.md / handoff.md):
  ...
  * This module only *maps* that verdict onto a gateway action using
    backend/policy/detection/disposition.yaml — it never re-scores or lets AI write disposition.
...
Used by:
  ...
  * workers that persist a PipelineResult to the spool
"""
...
from .paths import RULES_DETECTION, RULES_RUNTIME
from backend.stores.mail_thread import headers_from_raw
from workers.pipeline.stage_summary import compact_stages

_RULES = RULES_DETECTION / "disposition.yaml"
_ENFORCE_MODE_FILE = RULES_RUNTIME / "enforcement_mode.yaml"
...
def _read_runtime_enforce_mode() -> Optional[str]:
    """Read the admin-written runtime override from backend/policy/runtime/enforcement_mode.yaml.
...
class LocalQuarantineClient:
    ...
    def apply(self, queue_id: str, raw: bytes, result: PipelineResult) -> str:
        ...
        if spoolmod.use_s3():
            spoolmod.put_eml(queue_id, raw, bucket)
            ...
            meta = {
                ...
                "subject": result.subject,
                "from": result.from_header,
                "to": getattr(result, "to_header", "") or "",
                "score": result.composite_score,
                "hard_override": result.hard_override,
                "threat_class": result.threat_class,
                "threat_confidence": result.threat_confidence,
            ...
            spoolmod.put_meta(queue_id, meta, bucket)
...
def get_default_enforcement_client(
    ...
    mode = enforce_mode or resolve_enforce_mode()
    root = quarantine_root or Path(get_settings().quarantine_root or "email/spool")
...
def _find_spool_entry(root: Path, queue_id: str,
                      buckets=("quarantine", "rejected", "released", "gmail")) -> Path:
...
def list_spool_entries(quarantine_root: Path,
                       buckets=("quarantine", "rejected", "released", "gmail")) -> list:
...
def reevaluate_spool_entry(quarantine_root: Path, queue_id: str,
    ...
    configures — the CLI/gateway behavior, unchanged. The dashboard
    re-evaluate path passes dashboard_content_provider() so scoring uses
    the same LLM as live analysis.
    """
    ...
    result = run_pipeline(
        raw, source="smtp_hold", content_provider=content_provider, llm_triage=False,
    )
    ai_summary, ai_provider, ai_model = "", "", ""
    ai_structure = {}
    for st in getattr(result, "stages", None) or []:
        if getattr(st, "stage", None) != "content_ai":
            continue
        facts = getattr(st, "facts", None) or {}
        ai_summary = facts.get("summary") or ""
        ai_provider = facts.get("provider") or ""
        ai_model = facts.get("model_id") or ""
        ai_structure = {
    ...
    meta["stages"] = compact_stages(result)
    meta.update(ai_structure)
    meta_path.write_text(json.dumps(meta, indent=2, ensure_ascii=False) + "\n",
                         encoding="utf-8")
