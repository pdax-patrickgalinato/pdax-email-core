...
_PAREN_IPV4_RE = re.compile(r"\((\d{1,3}(?:\.\d{1,3}){3})\)")
_FROM_HOST_RE = re.compile(r"(?i)^from\s+(\S+)")
...
def _clean_from_host(host: str) -> str:
    host = (host or "").strip().rstrip(".")
    low = host.lower()
    if not host or low in ("unknown", "by") or low.startswith("unknown"):
        return ""
    if re.match(r"^\d{1,3}(?:\.\d{1,3}){3}", host):
        return ""
    if "trendmicro" in low:
        return ""
    return host


def _is_inbound_gateway(host: str) -> bool:
    """True when this hop's from-host is the org's inbound SEG/Google MX,
    not the sending MTA. Workspace mail often shows Trend TMES connecting
    to mx.google.com as a later hop — that IP is Trend's, not the origin."""
    h = (host or "").lower()
    return "tmes.trendmicro" in h
...
class ParsedEmail:
    ...
    def received_hops(self) -> list[dict]:
        """Newest-first Received hops with public IPv4s and a from-host."""
        hops: list[dict] = []
        for received in self.msg.get_all("Received") or []:
            text = " ".join(str(received).split())
            host = ""
            m = _FROM_HOST_RE.match(text)
            if m:
                host = m.group(1).strip().rstrip(".")
            ips: list[str] = []
            for ip in _BRACKETED_IP_RE.findall(text):
                if _is_public_ip(ip) and ip not in ips:
                    ips.append(ip)
            if not ips:
                for ip in _PAREN_IPV4_RE.findall(text):
                    if _is_public_ip(ip) and ip not in ips:
                        ips.append(ip)
            hops.append({"from_host": host, "ips": ips})
        return hops
        ...
        for hop in self.received_hops():
            for ip in hop["ips"]:
                if ip not in seen:
                    seen.add(ip)
                    ips.append(ip)
        for ip in _BRACKETED_IP_RE.findall(self.header("X-Originating-IP")):
            if _is_public_ip(ip) and ip not in seen:
                ips.append(ip)
        return ips

    def originating_hop(self) -> dict:
        """Best single originating MTA: oldest public Received hop that is
        not the inbound SEG/Google MX, plus X-Originating-IP when present.
        Empty dict if none."""
        xraw = self.header("X-Originating-IP")
        xips = _BRACKETED_IP_RE.findall(xraw)
        if not xips:
            xips = re.findall(r"(\d{1,3}(?:\.\d{1,3}){3})", xraw)
        xip = next((i for i in xips if _is_public_ip(i)), "")
        hops = self.received_hops()
        ...
        for hop in reversed(hops):
            ...
            candidates.append({
                "ip": hop["ips"][0],
                "hostname": _clean_from_host(hop["from_host"]),
                "gateway": _is_inbound_gateway(hop["from_host"]),
            })
        chosen: dict = {}
        for cand in candidates:
            if cand["gateway"]:
                continue
            chosen = {"ip": cand["ip"], "hostname": cand["hostname"], "x_originating_ip": xip}
            break
        if not chosen and candidates:
            cand = candidates[0]
            chosen = {"ip": cand["ip"], "hostname": cand["hostname"], "x_originating_ip": xip}
        if not chosen and xip:
            chosen = {"ip": xip, "hostname": "", "x_originating_ip": xip}
        if chosen:
            chosen["all_public_ips"] = self.originating_ips()
        return chosen
