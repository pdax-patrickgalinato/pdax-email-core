...
_FLAG_DESCRIPTIONS = {
    ...
    "first_request_class_from_sender": "This recipient has never received this kind of request from this sender before — even a trusted channel can deliver a first-time payment, access, or account-control ask to the wrong person.",
    "first_request_class_for_recipient": "This recipient has never received this kind of request from any scanned sender.",
    "first_trusted_sender_to_mailbox": "A trusted/known-good sender is writing to this mailbox for the first time.",
    # Sender history
    ...
    "minimal_body_with_link_only": "The message body is essentially just a URL (no question, no explanation). First-contact link-only mail is a common lure wrapper even when the host is a real brand.",
    "first_contact_link_only": "A sender never seen before sent a URL with no surrounding context — floored to SUSPICIOUS for analyst review.",
...
_FLAG_PREFIX_DESCRIPTIONS = {
    ...
    "campaign_hash": "This attachment hash is shared with campaign {value} — the same payload appeared in other scanned emails.",
    "campaign_url_path": "This landing URL is shared with campaign {value} — the same destination appeared in other scanned emails.",
    "campaign_url_host": "This URL host is shared with campaign {value} — other senders used the same destination domain.",
    "campaign_content": "This message template is shared with campaign {value} — near-identical body text appeared in other scanned emails.",
    "campaign_subject": "This subject template is shared with campaign {value} — other senders used the same lure wording.",
    "campaign_fanout": "This Message-ID is part of campaign {value} — the same send reached multiple scanned inboxes.",
...
# TMES-parity policy category keys (backend/policy/detection/policy.yaml) -> display name, for
