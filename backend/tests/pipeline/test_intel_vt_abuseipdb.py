"""Unit tests for VTAbuseIPDBIntelClient (workers/pipeline/intel.py) — VirusTotal
...
from workers.pipeline.intel import IntelCache, VTAbuseIPDBIntelClient


def _hits_degraded(out):
    """IntelClient.check may return (hits, degraded) or (hits, degraded, quota)."""
    return out[0], out[1]
...
def test_no_keys_configured_degrades_honestly():
    ...
    hits, degraded = _hits_degraded(client.check(["evil.example"], [], [], []))
    assert hits == []
...
def test_vt_domain_clean():
    ...
    client = VTAbuseIPDBIntelClient(vt_api_key="fake-vt-key", cache=_tmp_cache(),
                                    http_get=http, vt_min_interval_seconds=0)
    hits, degraded = _hits_degraded(client.check(["ok.example"], [], [], []))
    assert hits == []
    assert degraded is False
...
def test_vt_hash_hit():
    ...
    client = VTAbuseIPDBIntelClient(vt_api_key="fake-vt-key", cache=_tmp_cache(),
                                    http_get=http, vt_min_interval_seconds=0)
    hits, _ = _hits_degraded(client.check([], [], [], ["deadbeef"]))
...
def test_vt_url_hit_encodes_url_id():
    ...
    client = VTAbuseIPDBIntelClient(vt_api_key="fake-vt-key", cache=_tmp_cache(),
                                    http_get=http, vt_min_interval_seconds=0)
    hits, _ = _hits_degraded(client.check([], [], ["https://evil.example/x"], []))
...
def test_abuseipdb_ip_hit_above_threshold():
    ...
    client = VTAbuseIPDBIntelClient(abuseipdb_api_key="fake-key", cache=_tmp_cache(),
                                    http_get=http, vt_min_interval_seconds=0)
    hits, degraded = _hits_degraded(client.check([], ["1.2.3.4"], [], []))
...
def test_abuseipdb_ip_below_threshold_not_a_hit():
    ...
    client = VTAbuseIPDBIntelClient(abuseipdb_api_key="fake-key", cache=_tmp_cache(),
                                    http_get=http, vt_min_interval_seconds=0)
    hits, _ = _hits_degraded(client.check([], ["5.6.7.8"], [], []))
    assert hits == []
...
def test_cache_hit_avoids_second_http_call():
    ...
    client2 = VTAbuseIPDBIntelClient(vt_api_key="fake-vt-key", cache=cache,
                                     http_get=http, vt_min_interval_seconds=0)
    hits, _ = _hits_degraded(client2.check(["evil.example"], [], [], []))
    assert hits == ["intel_domain:evil.example"]
