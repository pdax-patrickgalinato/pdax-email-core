"""Unit tests for the TMES-parity policy layer (backend/policy/detection/policy.yaml,
workers/pipeline/policy.py, and its wiring into verdict.score_and_verdict()).
...
from workers.pipeline import policy, runner, verdict
...
def test_e2e_web_reputation_disabled_suppresses_lookalike_override():
    r = _run_with_policy("phish-lookalike.eml", _disabled("web_reputation"))
    assert r.hard_override != "url_lookalike_domain"
...
def test_e2e_virtual_analyzer_toggle_has_no_observable_effect_yet():
    ...
    raw = (_FIXTURES / "clean-normal.eml").read_bytes()
