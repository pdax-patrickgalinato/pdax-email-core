...
def _client_as(role: str) -> TestClient:
    ...
    from backend.api.routers import analyze as analyze_module
...
def test_missing_credentials_returns_503():
    client = _client_as("analyst")
    with mock.patch("cli.eml_analysis_agent.resolve_glm_credentials_path",
...
def test_success_shape_with_mocked_agent():
    ...
    with mock.patch("cli.eml_analysis_agent.resolve_glm_credentials_path",
         ...
         mock.patch("cli.eml_analysis_agent.analyze_eml_bytes", return_value=fake_deep):
