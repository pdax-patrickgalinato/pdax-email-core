...
def _get_all_with_fallback(msg, canonical: str, fallbacks: list) -> str:
    ...
    SPF/DKIM/DMARC. Confirmed against captured TMES-relayed mail (agora.eml), which has no
...
confirmed-clean captured messages (historical corpus, not in-repo):
...
* **`agora.eml`** — a legitimate passwordless-login email from Agora
  ...
  (`data/eml-output/agora.md`) came back `SPF: UNKNOWN`, `DKIM: UNKNOWN`
...
against captured `.eml` files with the LLM disabled (pure deterministic
extraction), plus a synthetic unit suite (`backend/tests/pipeline/test_forensics.py`, 15 cases
...
* **It caught a real attachment threat the prior filename/hash-only pass could
  not.** One captured Account-registration message carries an attachment
