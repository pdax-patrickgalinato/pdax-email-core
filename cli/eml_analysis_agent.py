...
pipeline (`run_pipeline()` in workers/pipeline/runner.py): it produces a full
...
get a ready-to-use client — see workers/pipeline/content_ai.py's GLMProvider
...
Usage:
    python3 -m cli.eml_analysis_agent path/to/mail.eml
    python3 -m cli.eml_analysis_agent path/to/eml-dir --output-dir data/eml-output
    python3 -m cli.eml_analysis_agent --credentials credentials.json --model zai-org/glm-4.7-maas
"""
...
from workers.pipeline.content_ai import GLMProvider
from workers.pipeline import attachment_forensics, url_forensics
from backend.config import get_settings
from backend.paths import CREDENTIALS_PATH, DATA_DIR
...
BODY STRUCTURE (`content_analysis.body_structure`) — read the email text
itself. Do not treat Fw:/Re: in the subject as authoritative, and do not use
keyword/regex shortcuts. Decide whether the message looks forwarded (an
original wrapped in a new envelope) vs a genuine reply vs a single new
message. Split into: primary_content (what the current sender wrote now);
quoted_or_forwarded_content (prior thread or forwarded original);
footer_content (signature, legal/confidentiality notice, unsubscribe).
Score/assess the PRIMARY message. Quoted/forwarded history may be camouflage
or may BE the lure — say which. Ordinary signatures are not worth scoring
(footer_worth_assessing=false). Set true only if the footer itself looks
hostile (credential links, fake brand, QR/payload, mismatched legal entity).
...
nothing; use empty string/array/false/0 for anything not applicable):
{
  ...
  "content_analysis": {"summary": "", "category": "", "sentiment": "", "entities": {"people": [], "organizations": [], "dates_mentioned": [], "amounts_mentioned": []}, "action_items": [], "body_structure": {"is_forwarded": false, "is_reply": false, "primary_content": "", "quoted_or_forwarded_content": "", "footer_content": "", "footer_worth_assessing": false, "footer_assessment": ""}},
...
def _enrich_osint_and_landing(result: dict) -> None:
    ...
    from workers.pipeline import landing_fetch, rdap_client
...
def parse_eml(path: Path) -> dict:
    ...
    result = {
        ...
        "text_body": text_body.strip()[:12000],
        "html_body_snippet": html_body.strip()[:4000],
    ...
    if attachments:
        from cli.email_forensic_playbook import run_playbook
...
def render_markdown(eml_path: Path, analysis: dict, playbook: dict = None,
    ...
    _model_label = model_id or get_settings().glm_model_id
    lines = [
    ...
    body_st = content.get("body_structure") or {}
    if isinstance(body_st, dict) and any(body_st.get(k) for k in (
            "is_forwarded", "is_reply", "primary_content",
            "quoted_or_forwarded_content", "footer_content",
            "footer_worth_assessing", "footer_assessment")):
        kind = ("forwarded" if body_st.get("is_forwarded")
                else "reply" if body_st.get("is_reply")
                else "original message")
        footer_note = ("worth assessing"
                       if body_st.get("footer_worth_assessing")
                       else "ordinary boilerplate — not scored")
        lines += [
            "",
            "**Message structure (LLM):**",
            f"- Shape: {kind}",
            f"- Footer: {footer_note}",
        ]
        if body_st.get("footer_assessment"):
            lines.append(f"- Footer assessment: {body_st['footer_assessment']}")
        if body_st.get("primary_content"):
            lines += ["", "*Primary content:*", "", body_st["primary_content"]]
        if body_st.get("quoted_or_forwarded_content"):
            lines += ["", "*Quoted / forwarded:*", "",
                      body_st["quoted_or_forwarded_content"]]
        if body_st.get("footer_content"):
            lines += ["", "*Footer:*", "", body_st["footer_content"]]
...
def resolve_glm_credentials_path(credentials_path: Optional[str] = None) -> Path:
    ...
    s = get_settings()
    env = s.glm_credentials_path or s.google_application_credentials
    if env:
        return Path(env)
...
def analyze_eml_bytes(
    ...
    *,
    ...
    if max_tokens is None:
        env_tok = (get_settings().deep_max_tokens or "").strip()
...
def main():
    ...
    ap.add_argument("path",
                     help="an .eml file or a directory of .eml files")
    ap.add_argument("--output-dir", default=str(DATA_DIR / "eml-output"),
                     help="default: data/eml-output")
    ap.add_argument("--credentials", default=str(CREDENTIALS_PATH),
                     help="GCP service-account JSON key (default: credentials.json at repo root)")
