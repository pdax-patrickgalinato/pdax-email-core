...
- **Scopes** (least privilege — read only for now):
  ```
  https://www.googleapis.com/auth/gmail.readonly
  ```
  `gmail.readonly` allows reading a message and its labels. SEGS does **not**
  move or create labels in this phase. Do not grant `gmail.modify` until
  you explicitly want clawback.

> ⚠️ **Domain-wide delegation is powerful** — that service-account key can read
> any mailbox in your domain. Store the key `600`, owned by the
...
[ ] Domain-wide delegation authorized in Admin console (scope: gmail.readonly only)
