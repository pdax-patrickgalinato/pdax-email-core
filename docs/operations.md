...
- **Campaign patterns**: other copies that share a landing URL, attachment hash, or template (reference only — does not change the score)

The **Campaigns** page lists clusters that share a landing URL, attachment hash, or message template across different copies. This is reference-only — it does not change a message’s score. Click a cluster to see senders, mailboxes, and member copies.

The **Workers** page shows Gmail poll, sender-profile ingest, campaign clustering, and LLM retry. A short campaign preview on that page links to the full list.

---

## Senders

The **Senders** page is a per-address baseline, not a verdict. Click an address to see:

- **Where they send from** — usual network role, countries, and ASNs on CLEAN/LOW copies (VPN/hosting/country deviations are flagged on new mail).
- **What they typically send and receive** — request-class mix (payment, access, HR, and so on) in both directions.
- **Who they talk to** — top counterparties on the scanned relationship graph (writes-to and receives-from).
- **When they send** — hour-of-day (UTC) from the message Date header.

Unusual hour or a brand-new counterpart only **scores** when the baseline is ready (5+ CLEAN/LOW copies) **and** the copy is a high-risk ask (payment, access, or account control). Otherwise those signals stay advisory.

External senders often show received=0: we only count mail delivered to mailboxes we poll.
...
After tuning weights or lists, re-run analysis from the dashboard enforcement
tools or by dropping the copy back through the worker graph. Path B
`hold_consumer.py` has been removed.
...
Source files: dashboard lists (allow/block) plus committed identity lists under `backend/policy/identity/` (protected domains, VIPs, freemail, brands). Banned extensions live in `backend/policy/detection/banned_extensions.txt`.
...
SEGS detection weights are in `backend/policy/detection/weights.yaml`. Each finding type maps to a numeric score contribution.
...
The audit log is append-only within the application. CloudWatch Logs streams a copy automatically (90-day retention).
...
The shipper maintains byte offsets in `data/wazuh_shipper_offsets.json`. This ensures no records are re-shipped or skipped across restarts. If this file is deleted, the shipper re-ships all records from the beginning of each log file — safe (Wazuh deduplicates by timestamp + content) but inefficient.
