...
from backend.models import HeaderFacts, StageResult, StageStatus
...
from backend.stores.sender_identity import load_protected_domains
...
def auth_passed(pe: ParsedEmail) -> bool:
    """True when Authentication-Results reports SPF or DKIM pass.

    Used to avoid scoring ESP/Gmail Workspace infrastructure tells
    (Message-ID domain ≠ From, first-party tracking pixels) that look like
    spoofing but are how legitimate platforms send mail.
    """
    ar = pe.header("Authentication-Results") or pe.header("ARC-Authentication-Results") or ""
    return bool(re.search(r"\bspf=pass\b", ar, re.I) or re.search(r"\bdkim=pass\b", ar, re.I))
...
def run(pe: ParsedEmail, protected_domains: list[str] | None = None) -> StageResult:
    ...
    score = 0.0
    protected_set = {
        registrable_domain(p)
        for p in (protected_domains if protected_domains is not None else load_protected_domains())
    }
    ...
    if facts.reply_to_divergent:
        from_protected = bool(from_dom and from_dom in protected_set)
        # Org From + non-freemail Reply-To is ticketing/routing, not a handoff.
        if from_protected and not facts.reply_to_freemail:
            pass
        else:
    ...
    # Forged mail often uses a Message-ID domain that doesn't match From.
    # Legitimate ESPs (Gmail Workspace, SendGrid, SES, JumpCloud, …) also
    # do this on every message — so only score the mismatch when SPF/DKIM
    # did not pass. Auth-pass + MID mismatch is infrastructure, not spoofing.
