...
backend/policy/detection/policy.yaml) and enforces which categories are allowed to contribute to
scoring/hard-overrides. Built as a categorization of backend/report.py's
...
# (the same convention backend/report.py's _FLAG_PREFIX_DESCRIPTIONS already
...
# Defaults used when backend/policy/detection/policy.yaml is missing, or missing a category key.
...
def filter_flags(flags: list, policy_cfg: Optional[dict]) -> tuple:
    ...
    audit trail — see backend/policy/detection/policy.yaml's module comment."""
    if policy_cfg is None:
        return list(flags), []
    active, suppressed = [], []
    for flag in flags:
