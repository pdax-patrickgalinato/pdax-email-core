...
every other real enrichment provider in this codebase). First-contact history
is filled from the behavioral store when one is passed in (count of prior
emails from this From address).
"""
...
from .request_class import is_trusted_channel
...
# promote to a backend/policy txt file if this needs frequent, code-free tuning).
BRAND_DOMAINS = {
...
def run(pe: ParsedEmail, protected_domains: list[str], vip_names: list[str],
        rdap_lookup=None, correlation_store=None) -> StageResult:
    ...
    # sending domain is not a protected domain. Trusted SaaS / analyst-confirmed
    # From addresses are a known channel (Kissflow display "PDAX") — calm the
    # identity flag; content analysis still scores the request itself.
    display = pe.from_display.lower()
    protected_set = {registrable_domain(p) for p in protected_domains}
    trusted = is_trusted_channel(pe.from_addr or "", from_dom)
    facts["trusted_channel"] = trusted
    if display and from_dom not in protected_set and not trusted:
    ...
    if rdap_lookup is None and get_settings().rdap_lookup:
    ...
    facts["first_contact"] = None
    if correlation_store is not None and hasattr(correlation_store, "sender_prior_count"):
        try:
            facts["first_contact"] = int(
                correlation_store.sender_prior_count((pe.from_addr or "").lower())
            )
        except Exception:
            facts["first_contact"] = None
